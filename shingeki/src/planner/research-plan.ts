...
function step(id: string, title: string, role: string, description: string): Step & { title: string; role: string } {
  return { id, title, role, description };
}
...
export function planJapanTrip(taskId: string): Plan & { labels: string[] } {
  const steps = [
    step('jp-1', 'Flights', 'executor', 'Find realistic round-trip flight options to Japan under the total trip budget; list 2–3 gateways and rough USD.'),
    step('jp-2', 'Lodging', 'executor', 'Pick cities/nights distribution for 5 days; prioritize food neighborhoods; estimate lodging USD.'),
    step('jp-3', 'Food', 'critic', 'Build a food-forward daily outline (markets, izakaya, regional specialties) with rough meal costs.'),
    step('jp-4', 'Culture', 'critic', 'Add cultural depth: museums, neighborhoods, transit passes; justify tradeoffs vs budget.'),
    step('jp-5', 'Final plan', 'critic', 'Deliver final day-by-day itinerary under $1500 including flights with justification for each major choice.'),
  ];
  return {
    taskId,
    steps,
